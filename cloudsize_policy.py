...
Options:
  ...
  -s STRING, --search=STRING
                        ...
                        update takes place prior to searching. Can be combined
                        with -p argument
  -p POLICY, --policy=POLICY
                        Run cloudsize in search mode, matching POLICY name.
                        No db update takes place prior to searching. Can be
                        combined with -s argument
Example usage:

cloudsize.py (no arguments - performs db update)
cloudsize.py -d /ifs/some_name.db
cloudsize.py -s /ifs/foo
cloudsize.py -p sales_homedir_archive
cloudsize.py -s /ifs/home -p archive_120days
...
def make_papi_call(method, uri, body="", cluster_ip="127.0.0.1"):
    """
    Function to handle calls to PAPI
    """
    username=uname
    pword=upass
    port = 8080
...
def policy_map(polid):
    """Takes policy id from cbm tool output and maps to current file policy name"""
    # Get info for this policy
    status, reason, polinfo = make_papi_call("GET", "/platform/3/filepool/policies/" + polid)
    resp_view = json.loads(polinfo)
    # Return policy name from parsed PAPI response
    return resp_view['policies'][0]['name']


def get_policy(file):
    """Takes file path and returns associated file policy name"""
    # get the policy ID associated with the stub, using shell out to cbm tool
    output = subprocess.Popen(['isi_test_cpool_cbm', '-M', file], stdout=subprocess.PIPE).communicate()[0]
    polid = output[output.find('Policy ID:') + 11:output.find('\n', output.find('Policy ID:'))]
    # send the numeric ID through policy_map and return
    return policy_map(polid)
...
def searchmode(sdir="ifs", pol="None"):
    """Takes search string and/or policy name, queries sqlite db, prints output"""
    totalsize = 0
    polsize = {}
    count = 0
    ...
    if pol != "None":
        sqlcur.execute("SELECT size from files WHERE name LIKE ? and size != 'NA' and policy = ?",
                       ("%" + sdir + "%", pol))
        for row in sqlcur:
            totalsize = totalsize + int(row[0])
        print "Cloud file size in paths matching %s, policy %s: %s" % (sdir, pol, hsize(totalsize))
    else:
        sqlcur.execute("SELECT size, policy from files WHERE name LIKE ? and size != 'NA'",
                        ("%" + sdir + "%",))
        for row in sqlcur:
            count += 1
            totalsize = totalsize + int(row[0])
            if row[1] not in polsize:
                polsize[row[1]] = 0
            polsize[row[1]] += int(row[0])
        for policy in polsize:
            print "Cloud file size in paths matching %s, policy %s: %s" % (sdir, policy,
                                                                        hsize(polsize[policy]))
        print "Total for %s files: %s" % (str(count), hsize(totalsize))
...
def addjobs():
    """Creates the jobs table, if needed, checks job completion, returns list
    of jobs to be processed, and map of cloudpools to job engine job ids"""
...
def addfiles(jobidlist, jobmap):
    """For jobs provided by addjobs, get file list and process all files"""
    # Connect to the sqlitedb
    conn = sqlite3.connect(dbpath)
    sqlcur = conn.cursor()
    sqlcur.execute("CREATE TABLE IF NOT EXISTS 'files' (id text, name text, state text, size text," +
                   " offset text, job_id text, policy text, je_job text, UNIQUE(id, offset))")
...
p.add_option("--search", "-s", dest="searchdir", type="string",
             help="Run cloudsize in search mode, matching STRING. No db \
                update takes place prior to searching. Can be combined \
                with -p argument")
p.add_option("--policy", "-p", dest="polname", type="string",
             help="Run cloudsize in search mode, matching POLICY name. \
                No db update takes place prior to searching. Can be \
                combined with -s argument")
...
if options.searchdir:
    if options.searchdir and options.polname:
        searchmode(options.searchdir, options.polname)
    else:
        searchmode(options.searchdir)
elif options.polname:
    searchmode('ifs', options.polname)
else:
